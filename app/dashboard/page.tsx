...
const mockClosed: Position[] = [
  {
    ...
    taxUSD: 6.6,
    holders: 2004,
    txCount: { buy: 410, sell: 390 },
    scores: { scorex: 81.1, risk: 12, fomo: 70, pumpDumpProb: 0.18 },
...
export default function DashboardPage() {
  ...
  function handleStart() {
    // TODO: POST /api/bot/start mit level
    setBotStatus('PAPER');
  }
  function handleStop() {
    // TODO: POST /api/bot/stop
    setBotStatus('OFF');
  }

  useEffect(() => {
    if (!detail) setTimeWindow('1h');
  }, [detail]);
    ...
    <div className={styles.page}>
      ...
      <div className={styles.grid}>
        ...
        <div className={styles.sideCol}>
          <Card title="Freies Kapital">
            <div className={styles.valueBig}>$ 118.72</div>
          </Card>

          <TrafficLightCard className={styles.mt12} winrate={0.62} factor={1.8} color="green" />

          <Card title="PnL (24h)" className={styles.mt12}>
            <div className={classNames(styles.valueMed, ' ', styles.positive)}>+ $42.70</div>
          </Card>
        </div>
      </div>
      ...
      {detail && (
        <DetailDrawer
          ...
          onSell={() => alert('Sell Position (TODO API)')}
        />
      )}
    </div>
  );
  ...
  return (
    <div className={styles.topbar}>
      <div className={styles.leftGroup}>
        <button className={classNames(styles.btn, styles.btnStart)} onClick={onStart} disabled={status !== 'OFF'}>
          Agent starten
        </button>
        <button className={classNames(styles.btn, styles.btnStop)} onClick={onStop} disabled={status === 'OFF'}>
          Agent stoppen
        </button>
...
function DetailDrawer(props: {
  ...
  const { position, timeWindow, onTimeWindow, onClose, onSell } = props;
  const p = position;
  return (
...
function SettingsModal(props: { onClose: () => void }) {
  ...
  return (
    <div className={styles.modalOverlay} onClick={props.onClose}>
      <div className={classNames(styles.modal, styles.glassy)} onClick={e => e.stopPropagation()}>
        <div className={styles.modalHeader}>
          ...
          <div className={styles.settingsGrid}>
            ...
            <div className={styles.settingsCard}>
              ...
              {Object.entries(cats).map(([k, v]) => (
                <label key={k} className={styles.switch}>
                  <input type="checkbox" checked={v} onChange={e => setCats({ ...cats, [k]: e.target.checked })} />
                  <span>{k}</span>
                </label>
              ))}
            </div>
            ...
            <div className={styles.settingsCard}>
              ...
              {Object.entries(events).map(([k, v]) => (
                <label key={k} className={styles.switch}>
                  <input type="checkbox" checked={v} onChange={e => setEvents({ ...events, [k]: e.target.checked })} />
                  <span>{k}</span>
                </label>
              ))}
            </div>
...
function TaxExportPanel() {
  ...
  function download() {
    // TODO: GET /api/tax/export?from=&to= ; hier nur Mock-Text
    const lines = [
